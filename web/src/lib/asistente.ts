...
//
// Las respuestas que dependen de los datos del paciente (próximo turno, recetas por vencer)
// se arman con los mismos helpers que usan las páginas, así nunca dicen algo distinto de lo
// que el paciente ve en pantalla.
...
export const TEMAS: Tema[] = [
  // ── Turnos ──
  {
    ...
    responder: () => ({
      texto: 'Entrá a "Solicitar turno", elegí la especialidad, el médico y después la fecha y el horario que te queden bien. Al confirmar te queda registrado y lo vas a ver en "Mis turnos".',
      acciones: [{ label: 'Solicitar turno', to: '/paciente/solicitar' }],
    }),
  },
  {
    id: 'proximo-turno',
    ...
    responder: ({ turnos }) => {
      ...
      return {
        ...
        acciones: [{ label: 'Ver mis turnos', to: '/paciente/turnos' }],
      };
    },
  },
  {
    ...
    responder: ({ turnos }) => ({
      texto: turnos.length
        ...
        ? `En "Mis turnos" tenés ${plural(turnos.length, 'turno registrado', 'turnos registrados')}. Ahí se separan los próximos del historial, y hay un calendario mensual.`
        : 'En "Mis turnos" vas a ver tus próximos turnos y el historial. Todavía no tenés ninguno.',
      acciones: [{ label: 'Mis turnos', to: '/paciente/turnos' }],
    }),
  },
  {
    id: 'cancelar-turno',
    ...
    responder: () => ({
      texto: 'En "Mis turnos" buscá el turno pendiente y tocá "Cancelar turno". Podés dejar el motivo si querés. Avisá con tiempo así el horario le queda libre a otra persona.',
      acciones: [{ label: 'Mis turnos', to: '/paciente/turnos' }],
    }),
  },
  {
    ...
    responder: () => ({
      texto: 'No se puede editar un turno ya sacado. Lo que hacés es cancelarlo desde "Mis turnos" y sacar uno nuevo en el horario que te sirva.',
      acciones: [
        { label: 'Mis turnos', to: '/paciente/turnos' },
        { label: 'Solicitar turno', to: '/paciente/solicitar' },
      ],
    }),
  },
  {
  ...
  // ── Recetas ──
  {
    ...
    responder: ({ recetas }) => {
      ...
      if (!recetas.length) {
        return {
          texto: 'Todavía no tenés recetas. Cuando un médico te emita una, la vas a ver en "Recetas" con su fecha de vencimiento.',
          acciones: [{ label: 'Mis recetas', to: '/paciente/recetas' }],
        };
      }
      return {
        texto: `Tenés ${plural(vigentes.length, 'receta vigente', 'recetas vigentes')} de ${plural(recetas.length, 'receta en total', 'recetas en total')}. En "Recetas" podés filtrarlas por estado, buscarlas por medicamento e imprimirlas.`,
        acciones: [{ label: 'Mis recetas', to: '/paciente/recetas' }],
      };
    },
  },
    ...
    responder: ({ recetas }) => {
      ...
      if (porVencer.length) {
        ...
        return {
          ...
          acciones: [{ label: 'Ver las que vencen', to: '/paciente/recetas' }],
        };
      }
      ...
      if (vencidas.length && vencidas.length === recetas.length) {
        return {
          ...
          acciones: [
            { label: 'Solicitar turno', to: '/paciente/solicitar' },
            { label: 'Mis recetas', to: '/paciente/recetas' },
          ],
        };
      }

      return {
        texto: `Ninguna de tus recetas vence en los próximos ${DIAS_POR_VENCER} días. En "Recetas" cada una muestra hasta cuándo es válida, y las que están por vencer aparecen en amarillo.`,
        acciones: [{ label: 'Mis recetas', to: '/paciente/recetas' }],
      };
    },
  },
  {
    id: 'imprimir-receta',
    titulo: '¿Cómo imprimo o descargo una receta?',
    categoria: 'recetas',
    palabras: ['imprimir', 'impresion', 'impresión', 'descargar', 'pdf', 'papel', 'farmacia', 'guardar receta'],
    responder: () => ({
      texto: 'En "Recetas", tocá el ícono de impresora de la receta que necesites. Se abre una vista lista para imprimir; desde ahí también podés elegir "Guardar como PDF" en el diálogo de impresión.',
      acciones: [{ label: 'Mis recetas', to: '/paciente/recetas' }],
    }),
  },
  ...
  // ── Estudios ──
  {
    ...
    responder: () => ({
      texto: 'Entrá a "Mis estudios" y tocá "Subir estudio". Podés arrastrar el archivo o elegirlo desde tu equipo, y le ponés un título para encontrarlo después.',
      acciones: [{ label: 'Mis estudios', to: '/paciente/estudios' }],
    }),
  },
  {
    id: 'formatos-estudio',
  ...
  {
    id: 'ver-estudios',
    titulo: '¿Dónde veo o borro mis estudios?',
    categoria: 'estudios',
    palabras: ['ver estudios', 'mis estudios', 'borrar', 'eliminar', 'sacar', 'archivos', 'guardados'],
    responder: ({ estudios }) => ({
      texto: estudios.length
        ? `Tenés ${plural(estudios.length, 'estudio guardado', 'estudios guardados')}. Desde "Mis estudios" los abrís con "Ver", y con el ícono de tacho borrás los que hayas subido por error.`
        : 'Todavía no subiste estudios. Cuando subas alguno vas a poder abrirlo o borrarlo desde "Mis estudios".',
      acciones: [{ label: 'Mis estudios', to: '/paciente/estudios' }],
    }),
  },
  ...
  // ── Cuenta ──
  {
    ...
    responder: () => ({
      texto: 'Andá a "Mi perfil" y tocá "Cambiar contraseña": te pide la actual y la nueva. La nueva tiene que tener al menos 8 caracteres, con una letra y un número. Si no te acordás de la actual, cerrá sesión y usá "¿Olvidaste tu contraseña?" en la pantalla de ingreso.',
      acciones: [{ label: 'Mi perfil', to: '/paciente/perfil' }],
    }),
  },
  {
    id: 'editar-datos',
    ...
    responder: () => ({
      texto: 'En "Mi perfil" tocá "Editar datos" para cambiar tu nombre, apellido y teléfono. El email y el DNI no se pueden modificar por tu cuenta: escribile a la administración del centro si hay un error.',
      acciones: [{ label: 'Mi perfil', to: '/paciente/perfil' }],
    }),
  },
  {
    ...
    responder: () => ({
      texto: 'Verificar tu email confirma que la casilla es tuya y nos deja mandarte los avisos de turnos y recetas. En "Mi perfil", en la tarjeta de seguridad, también podés activar la verificación en dos pasos: además de la contraseña te va a pedir un código cada vez que entres.',
      acciones: [{ label: 'Mi perfil', to: '/paciente/perfil' }],
    }),
  },
  ...
  // ── Centros ──
  {
    ...
    responder: () => ({
      ...
      acciones: [{ label: 'Centros y mapa', to: '/paciente/mapa' }],
    }),
  },
];
